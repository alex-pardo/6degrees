...
\begin{abstract}
The six degree of separation is the most popular theory in social networks. Exists a large number of algorithm that tries to check this theory in a different types of networks. Otherwise, the Ant Colony Optimization is an approach of a computational intelligence method based on the swarm intelligence. Small individuals ants with few capabilities do a emerge behaviour able to find the shortest path between two points.

The next project try to join these two concepts: the six degree of separation as a Ant Colony Optimization problem in one. We will code the algorithm and to see how this algorithm works in a two socials networks like Twitter and BlogCatalog.
\end{abstract}
...
\begin{figure}[htb!]
\centering
\includegraphics[scale=0.3]{img/big.png}
\label{fig:graph3}
\caption{Third graph used}
\end{figure}
...
\item \textbf{Choice the next nodes}: We use a probabilistic method to obtain the next node at each turn. The different options can have pheromone and the sum of pheromone of the all options are one. We build and array with a size N based on the probabilities of the all options. The next step is randomly selects some cell of the array and check which option of the path is. The ants should be follow this option in this turn.
...
\item \textbf{Ants at each turn}: The algorithm does not start with all the ants in the start point. At each turn, the algorithm add more ants in the start point. Also, we add a parameter named \emph{NUM\_ANTS} to control the maximum number of the ants in the whole system.
